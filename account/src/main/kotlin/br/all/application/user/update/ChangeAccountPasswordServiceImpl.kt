    ...
    override fun changePassword(
        ...
        if (userCredentials == null) {
            presenter.prepareFailView(
                NoSuchElementException("User with id ${request.userId} doesn't exist!")
            )
            return
        }
        ...
        if (!encoder.matches(request.oldPassword, userCredentials.password)) {
            presenter.prepareFailView(
                IllegalArgumentException("Invalid old password provided!")
            )
            return
        }
        ...
        if (request.newPassword != request.confirmPassword) {
            presenter.prepareFailView(
                IllegalArgumentException("Confirm password does not match new password!")
            )
            return
        }
        ...
        if (encoder.matches(request.newPassword, userCredentials.password)) {
            presenter.prepareFailView(
                IllegalArgumentException("New password cannot be the same as the old password!")
            )
            return
        }

        try {
            updatePasswordService.update(request.userId, request.newPassword)

            presenter.prepareSuccessView(
                ChangeAccountPasswordService.ResponseModel(request.userId)
            )
        } catch (e: Exception) {
