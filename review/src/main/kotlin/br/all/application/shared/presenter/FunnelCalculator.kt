...
object FunnelCalculator {
    fun calculate(studies: List<StudyReviewDto>): FunnelExportData {
        val nonDuplicated = studies.filter {
            it.selectionStatus != SelectionStatus.DUPLICATED.name
        }

        return FunnelExportData(
            totalIdentifiedBySource = studies
                .flatMap { it.searchSources }
                .groupingBy { it }
                .eachCount(),

            totalAfterDuplicatesRemovedBySource = nonDuplicated
                .flatMap { it.searchSources }
                .groupingBy { it }
                .eachCount(),
            ...
            totalExcludedInScreening = studies.count {
                it.selectionStatus == SelectionStatus.EXCLUDED.name
            },

            excludedByCriterion = studies
                .filter {
                    it.selectionStatus == SelectionStatus.EXCLUDED.name
                }
            ...
            totalFullTextAssessed = studies.count {
            ...
            totalExcludedByCriterion = studies
                .filter {
                    it.selectionStatus == SelectionStatus.INCLUDED.name &&
                            it.extractionStatus == ExtractionStatus.EXCLUDED.name
                }
