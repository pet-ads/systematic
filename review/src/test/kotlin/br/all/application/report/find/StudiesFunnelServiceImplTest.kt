...
class StudiesFunnelServiceImplTest {
    ...
    inner class WhenBeingSuccessful {
        ...
        fun `should return correct response for multiple studies`() {
            ...
            every { review1.extractionStatus } returns ExtractionStatus.EXCLUDED.name

            every { review1.selectionCriteria } returns setOf(
                "Crit1",
                "Crit2"
            )

            every { review1.extractionCriteria } returns emptySet()
            ...
            every { review2.selectionCriteria } returns emptySet()

            every { review2.extractionCriteria } returns setOf(
                "Crit2",
                "Crit3"
            )
            ...
            every { review3.selectionCriteria } returns emptySet()

            every { review3.extractionCriteria } returns emptySet()
            ...
            every { review4.extractionStatus } returns ExtractionStatus.DUPLICATED.name

            every { review4.selectionCriteria } returns emptySet()

            every { review4.extractionCriteria } returns emptySet()
            ...
            val expectedResponse = StudiesFunnelService.ResponseModel(
                ...
                totalFullTextAssessed = 2,

                totalExcludedInFullText = 1,
